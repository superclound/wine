  ...
  <div class="shopping">
    <syy-BScroll ref="syyscroll">
      <div class="sss_10">
        ...
        <div class="list">
          <ul>
            <li v-for="(item,index) in splist3" :key="index">
              <div class="img">
                <img :src="item.proImg">
              </div>
              <div class="img_right">
...
<script>
import { list3_time_api } from "../../api/list";
import { list3_3_time_api } from "../../api/list";
...
export default {
  ...
  async created() {
    if (!sessionStorage.getItem("splist3")) {
      let data = await list3_time_api();
      this.splist3 = data.data.killProList;
      let { hourThree } = data.data.seckillDate;
      this.time = hourThree;
      sessionStorage.setItem("splist3", JSON.stringify(data.data.killProList));
    }
  },
  mounted() {
 ...
 // // 下拉刷新   
    this.$refs.syyscroll.handlepullingDown(async () => {
      ...
      let data = await list3_3_time_api();
      this.splist3 = data.data.killProList;
    ...
    this.$refs.syyscroll.handlepullingUp( async() => {    
      ...
      let data = await list3_3_time_api();
      this.splist3 = [...this.splist3, ...data.data.killProList];
      sessionStorage.setItem("splist3", JSON.stringify(data.data.killProList));
  ...
  data() {
    return {
      splist3: JSON.parse(sessionStorage.getItem("splist3")) || [],
      flag: 0,
      time: "",
      h: "",
      m: "",
