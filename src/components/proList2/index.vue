  ...
  <div class="shopping">
    <syy-BScroll ref="syyscroll">
      <div class="action">
        ...
        <div class="list">
          <ul>
            <li v-for="(item,index) in splist2" :key="index">
              <div class="img">
                <img :src="item.proImg">
              </div>
              <div class="img_right">
...
import { list2_time_api } from "../../api/list";

import { list2_2_time_api } from "../../api/list";

export default {
  name: "List",
  //   async 异步 可直接获取.then中的值  不用.then获取
  async created() {
    if (!sessionStorage.getItem("splist2")) {
      let data = await list2_time_api();
      this.splist2 = data.data.killProList;
      let { hourTwo } = data.data.seckillDate;
      this.time = hourTwo;
      sessionStorage.setItem("splist2", JSON.stringify(data.data.killProList));
    }
  },
  mounted() {
   ...
   // // 下拉刷新   
    this.$refs.syyscroll.handlepullingDown(async () => {
      ...
      let data = await list2_2_time_api();
      this.splist2 = data.data.killProList;
      sessionStorage.setItem("splist2",JSON.stringify(data.data.killProList));
    ...
    this.$refs.syyscroll.handlepullingUp( async() => {    
      ...
      let data = await list2_2_time_api();
      this.splist2 = [...this.splist2, ...data.data.killProList];
      sessionStorage.setItem("splist2", JSON.stringify(data.data.killProList));
  ...
  data() {
    return {
      ...
      flag: 0,
      time: "",
      splist2: JSON.parse(sessionStorage.getItem("splist2")) || []
    };
  }
};
</script>
